...
search_entry.bind("<KeyRelease>", lambda event: apply_filters(price_combobox.get(), rooms_combobox.get(), search_entry.get(), sort_combobox.get()))
...
def binary_search(dest_buttons, keyword):
    low, high = 0, len(dest_buttons) - 1
    while low <= high:
        mid = (low + high) // 2
        destination = dest_buttons[mid].original_text.lower()
        if keyword.lower() in destination:
            return mid
        elif keyword.lower() < destination:
            high = mid - 1
        else:
            low = mid + 1
    return -1
...
# Modify the apply_filters function to use binary search for the keyword filter
...
# Modify the update_destination_buttons function to use binary search result
def update_destination_buttons(filtered_destinations, price_filter, rooms_filter):
    # Use binary search for keyword filter
    keyword_index = binary_search(destination_buttons, search_entry.get())
    ...
    for i, button in enumerate(destination_buttons):
        ...
        # Check if the destination is in the filtered list and matches the keyword
        if destination in filtered_destinations and (i == keyword_index or keyword_index == -1):
