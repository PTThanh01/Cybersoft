...
def apply_sort_filter(sort_option):
    if "Name (A-Z)" in sort_option:
        quicksort_destinations_by_name(destination_buttons, "asc")
    elif "Name (Z-A)" in sort_option:
        quicksort_destinations_by_name(destination_buttons, "desc")
    elif "Price (Low to High)" in sort_option:
        quicksort_destinations_by_price(destination_buttons, "asc")
    elif "Price (High to Low)" in sort_option:
        quicksort_destinations_by_price(destination_buttons, "desc")
...
def quicksort_destinations_by_name(dest_buttons, sort_order):
    locale.setlocale(locale.LC_COLLATE, 'vi_VN.utf8')

    def partition(low, high):
        pivot_index = (low + high) // 2
        dest_pivot = dest_buttons[pivot_index].cget("text")
        i = low - 1
        j = high + 1

        while True:
            i += 1
            while (sort_order == "asc" and locale.strcoll(dest_buttons[i].cget("text"), dest_pivot) < 0) or \
                  (sort_order == "desc" and locale.strcoll(dest_buttons[i].cget("text"), dest_pivot) > 0):
                i += 1

            j -= 1
            while (sort_order == "asc" and locale.strcoll(dest_buttons[j].cget("text"), dest_pivot) > 0) or \
                  (sort_order == "desc" and locale.strcoll(dest_buttons[j].cget("text"), dest_pivot) < 0):
                j -= 1

            if i >= j:
                return j

            dest_buttons[i], dest_buttons[j] = dest_buttons[j], dest_buttons[i]

    def quicksort(low, high):
        if low < high:
            partition_index = partition(low, high)
            quicksort(low, partition_index)
            quicksort(partition_index + 1, high)

    n = len(dest_buttons)
    quicksort(0, n - 1)

def quicksort_destinations_by_price(dest_buttons, sort_order):
    def partition(low, high):
        pivot_index = (low + high) // 2
        dest_pivot = dest_buttons[pivot_index].cget("text")
        price_pivot = extract_price_from_text(dest_pivot)
        ...
        while True:
            i += 1
            while (sort_order == "asc" and extract_price_from_text(dest_buttons[i].cget("text")) < price_pivot) or \
                  (sort_order == "desc" and extract_price_from_text(dest_buttons[i].cget("text")) > price_pivot):
                i += 1

            j -= 1
            while (sort_order == "asc" and extract_price_from_text(dest_buttons[j].cget("text")) > price_pivot) or \
                  (sort_order == "desc" and extract_price_from_text(dest_buttons[j].cget("text")) < price_pivot):
                j -= 1
...
def extract_price_from_text(text):
    ...
    for line in lines:
        if line.startswith("Giá:"):
            # Extract the numeric part of the price (remove "$" sign) and convert to integer
            return int(''.join(char for char in line.split(":")[1].strip() if char.isdigit()))
